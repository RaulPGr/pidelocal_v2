...
export async function GET(req: Request) {
  ...
  let qProd = supabaseAdmin
    .from(TABLE)
    .select('*, product_weekdays(day), allergens')
  ...
  try {
    ...
    if (bidP) {
      ...
      // SECURITY: If we can't determine the business, do NOT return all products.
...
export async function PATCH(req: Request) {
  ...
  const updates: any = {};
  for (const key of ['name', 'description', 'price', 'image_url', 'available', 'category_id', 'allergens']) {
