...
export async function GET(req: NextRequest) {
  try {
    ...
    // Filter out unpaid Stripe orders (they are just "intents" until paid)
    const data = (rawData || []).filter((o: any) => {
      const isStripe = o.payment_method === 'stripe' || o.payment_method === 'card';
      const isPaid = o.payment_status === 'paid';
      if (isStripe && !isPaid) return false;
      return true;
    });
