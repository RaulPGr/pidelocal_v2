...
export async function GET(req: Request) {
  try {
    ...
    // Obtener imágenes de productos (manual join para evitar errores de FK)
    ...
    if (productIds.length > 0) {
      ...
      products?.forEach((p) => {
        if (p.image) productImagesMap.set(p.id, p.image);
      });
    }
    ...
    const normalizedItems =
      (items || []).map((item) => ({
        ...
        image: productImagesMap.get(item.product_id) || null,
