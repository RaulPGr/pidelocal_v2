...
export async function POST(req: NextRequest) {
  try {
    ...
    productGroupAssignments.forEach((row) => {
      if (!allowedGroupsByProduct.has(row.product_id)) allowedGroupsByProduct.set(row.product_id, new Set());
      allowedGroupsByProduct.get(row.product_id)!.add(row.group_id);
    });
    const allGroupIds = Array.from(new Set(productGroupAssignments.map((pg) => pg.group_id)));

    // Categoría -> Grupos
    const categoryGroupAssignments: Array<{ category_id: number; group_id: string }> = categoryIds.length
      ? ((await supabaseAdmin
    ...
    categoryGroupAssignments.forEach((row) => {
      const pids = productsByCategory.get(row.category_id) || [];
      pids.forEach((pid) => {
        if (!allowedGroupsByProduct.has(pid)) allowedGroupsByProduct.set(pid, new Set());
        allowedGroupsByProduct.get(pid)!.add(row.group_id);
    ...
    // Opciones
    const groups: any[] = uniqueGroupIds.length
      ? ((await supabaseAdmin
        .from("option_groups")
        .select("id, name, min_select, max_select, is_required, selection_type")
        .in("id", uniqueGroupIds)) as any).data || []
      : [];
    const groupMetaMap = new Map(groups.map((g) => [g.id, g]));

    const options_db: any[] = uniqueGroupIds.length
      ? ((await supabaseAdmin
        .from("options")
        .select("id, name, price_delta, group_id")
        .in("group_id", uniqueGroupIds)) as any).data || []
      : [];
    const optionRowMap = new Map(options_db.map((o) => [o.id, o]));

    let subtotalCents = 0;
    const itemsPrepared = body.items.map((i) => {
      const p = map.get(i.productId);
      if (!p) throw new Error(`Producto ID ${i.productId} no encontrado`);
    ...
    let deliveryAddress = '';
    const isDelivery = body.delivery?.type === 'delivery';
    if (isDelivery) {
      // Read fee from payload or recalculate? 
      // For security, ideally recalculate, but since we rely on payload for 'subtotal' validation in frontend...
      // We will read from pricing.deliveryFee if available, OR we should fetch settings.
      // Let's trust payload structure for delivery fee if provided by client (simple approach), 
      // OR better: fetch settings like we do for products?
      // Fetching settings is safer.
      const s = (tenant as any)?.social;
      if (s?.delivery_enabled) {
        const fee = Number(s.delivery_fee || 0);
        const min = Number(s.delivery_min || 0);
        // Calculate subtotal euros
        const subEur = subtotalCents / 100;
        if (subEur >= min) {
          deliveryFeeCents = Math.round(fee * 100);
        }
      }
      if (body.delivery?.address) {
        deliveryAddress = String(body.delivery.address).trim();
      }
    }

    const finalTotalCents = Math.max(0, subtotalCents + deliveryFeeCents - discountCents);
    ...
    // Decidir estado inicial: 'pending' (pending_payment si es tarjeta, pending_acceptance si es cash)
    const initialStatus = 'pending';

    // Prepare Notes with Address
    let finalNotes = body.notes || '';
    if (isDelivery && deliveryAddress) {
      finalNotes = `[ENTREGA DOMICILIO] Dirección: ${deliveryAddress}\n${finalNotes}`;
    } else if (isDelivery) {
      finalNotes = `[ENTREGA DOMICILIO] (Sin dirección detallada)\n${finalNotes}`;
    }
    ...
    if (body.paymentMethod === 'card') {
      const stripeAccountId = (tenant as any)?.social?.stripe_account_id;
      if (!stripeAccountId) {
        throw new Error("El negocio no tiene configurados los pagos online.");
      }

      const { stripe } = await import('@/lib/stripe');

      // Items for Stripe
      const line_items = itemsPrepared.map(it => {
        const opts = (it.options_snapshot || []).map(o => o.name_snapshot).join(', ');
        return {
          price_data: {
            currency: 'eur',
            product_data: {
              name: it.name,
              description: opts || undefined,
            },
        ...
        line_items.push({
          price_data: {
            ...
            product_data: {
              ...
              description: deliveryAddress ? `Dirección: ${deliveryAddress}`.slice(0, 500) : 'Entrega en tu dirección',
            },
      ...
      const origin = req.headers.get('origin') || 'https://pidelocal.com';
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: `${origin}/order/${orderId}?status=success`,
        cancel_url: `${origin}/order/${orderId}?status=cancelled`,
      }, {
      ...
      if (session.url) {
        ...
        // Checkpoint: Store session ID immediately to allow manual sync later
        await supabaseAdmin
          .from('orders')
    ...
    if (body.paymentMethod === 'cash') {
      ; (async () => {
        try {
          ...
          console.error('[email] create-order fallo:', e);
        }
      })();
    ...
    return NextResponse.json({ ok: true, orderId, code, checkoutUrl });
  } catch (err: any) {
    return NextResponse.json(
